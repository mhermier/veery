...
class Lexer {
  ...
  readToken() {
    ...
    if (c == Character.fromCodePoint(47)) {
      if (match(Character.fromCodePoint(47))) return readLineComment()
  ...
  readNumber() {
    ...
    if (peek() == Character.fromCodePoint(46) && peek(1) != null && peek(1).isDigit) {
    ...
    if (match(Character.fromCodePoint(101))) {
      if (peek() == Character.fromCodePoint(43) || peek() == Character.fromCodePoint(45)) {
        advance()
      }
      while (match{|c|
        return c.isDigit
  ...
  match(condition) {
    ...
    if (condition is Character) {
      ...
      Abort.internal_error("Unsupported condition type")
    }
    advance()
    return true
  }
